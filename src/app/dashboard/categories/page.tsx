...
export default function Page() {
  ...
  useEffect(() => { load(); }, []);
  async function load() { setLoading(true); const r = await fetch('/api/categories'); const d = await r.json(); setItems(d.categories || []); setLoading(false); }
  ...
  async function handleSave() {
    setError('');
    if (editItem) {
      const res = await fetch('/api/categories', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: editItem.id, name }) });
  ...
  async function handleDelete(item: any) {
    ...
    const res = await fetch('/api/categories', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: item.id }) });
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end"><Button onClick={openCreate}><Plus className="w-4 h-4" />Add Category</Button></div>
      {loading ? <div className="bg-white rounded-2xl p-12 text-center text-gray-400">Loading...</div> : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {items.map((item: any) => (
            <div key={item.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-center justify-between hover:shadow-md transition">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-red-50 flex items-center justify-center"><FolderOpen className="w-5 h-5 text-red-600" /></div>
          ...
          {items.length === 0 && <div className="col-span-3 bg-white rounded-2xl p-12 text-center text-gray-400">No categories yet</div>}
        </div>
      )}
