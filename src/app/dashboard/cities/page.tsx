...
export default function Page() {
  ...
  useEffect(() => { load(); }, []);
  async function load() { setLoading(true); const r = await fetch('/api/cities'); const d = await r.json(); setItems(d.cities || []); setLoading(false); }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end"><Button onClick={() => setModal(true)}><Plus className="w-4 h-4" />Add City</Button></div>
          ...
          {items.map((item: any) => (
            <div key={item.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-center justify-between hover:shadow-md transition">
              ...
              <div className="flex items-center gap-1">
                <button className="p-1.5 rounded-lg hover:bg-gray-100"><Edit className="w-4 h-4 text-gray-400" /></button>
                <button className="p-1.5 rounded-lg hover:bg-red-50"><Trash className="w-4 h-4 text-red-400" /></button>
              </div>
            </div>
          ))}
          {items.length === 0 && <div className="col-span-3 bg-white rounded-2xl p-12 text-center text-gray-400">No cities yet</div>}
        </div>
      )}
      <Modal open={modal} onClose={() => setModal(false)} title={'Add City'}>
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3"><Button variant="secondary" onClick={() => setModal(false)}>Cancel</Button><Button onClick={handleCreate}>Save</Button></div>
        </div>
      </Modal>
    </div>
  );
